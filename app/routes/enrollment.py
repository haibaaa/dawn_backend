...
def validate_target_score(target_score: float | None):
    if target_score is not None and not 0 <= target_score <= 100:
        raise HTTPException(
            status_code=400, detail="target_score must be between 0 and 100"
        )
...
@router.get(
    "/", response_model=List[schemas.EnrollmentOut], response_model_exclude_none=True
)
def get_enrollments(
    db: Session = Depends(get_db), user: models.User = Depends(get_and_sync_user)
):
    return (
...
@router.get(
    "/{id}", response_model=schemas.EnrollmentOut, response_model_exclude_none=True
)
def get_enrollment(
    id: int,
...
def create_enrollment(
    ...
    db: Session = Depends(get_db),
    valid_courses: dict = Depends(get_valid_courses),
    user: models.User = Depends(get_and_sync_user),
):
...
def update_target(
    id: int,
    targets: schemas.EnrollmentUpdate,
    db: Session = Depends(get_db),
    user: models.User = Depends(get_and_sync_user),
):
