...
from app.utils import check_enrolled, get_and_sync_user, get_current_user
...
def get_assessment_or_404(db: Session, id: int) -> models.Assessment:
    ...
    if not assessment:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"assessment {id} not found"
        )
    return assessment
...
def validate_assessment_fields(max_score: float | None, deadline: datetime | None):
    if max_score is not None and max_score <= 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="max_score must be positive"
        )
...
def create_assessment(
    ...
    group = (
        db.query(models.AssessmentGroup)
        .filter(models.AssessmentGroup.id == assessment.assessment_group_id)
        .first()
    )
    ...
    if not group:
        raise HTTPException(
            status_code=404,
            detail=f"Assessment group {assessment.assessment_group_id} not found",
        )
...
@router.patch(
    "/{id}", response_model=schemas.AssessmentOut, response_model_exclude_none=True
)
def update_assessment(
    id: int,
    updated_assessment: schemas.AssessmentUpdate,
    db: Session = Depends(get_db),
    user: models.User = Depends(get_and_sync_user),
):
    ...
    validate_assessment_fields(
        updated_data.get("max_score"), updated_data.get("deadline")
    )
