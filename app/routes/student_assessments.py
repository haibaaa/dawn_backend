...
def get_student_assessment_or_404(
    db: Session, user_id: str, assessment_id: int
    ...
    if not sa:
        raise HTTPException(
            status_code=404, detail=f"Score for assessment {assessment_id} not found"
        )
    return sa
...
@router.get(
    "/",
    response_model=List[schemas.StudentAssessmentOut],
...
@router.post(
    "/",
    response_model=schemas.StudentAssessmentOut,
    status_code=201,
    response_model_exclude_none=True,
)
def record_score(
    student_assessment: schemas.StudentAssessmentCreate,
    db: Session = Depends(get_db),
    user: models.User = Depends(get_and_sync_user),
):
    ...
    if existing:
        raise HTTPException(
            ...
            detail="Score already recorded for this assessment — use PATCH to update",
        )
    ...
    assessment = (
        db.query(models.Assessment)
        .filter(models.Assessment.id == student_assessment.assessment_id)
        .first()
    )
    if not assessment:
    ...
    if assessment.max_score and student_assessment.score > assessment.max_score:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Score cannot exceed max score of {assessment.max_score}",
        )
...
def update_score(
    assessment_id: int,
    updated: schemas.StudentAssessmentUpdate,
    db: Session = Depends(get_db),
    user: models.User = Depends(get_and_sync_user),
):
