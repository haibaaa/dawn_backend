...
from .models import Task, Enrollment, TaskDependency, Status
...
class PriorityEngine:
    ...
    def calculate_task_priority(db: Session, task_id: int):
        ...
        task = db.query(Task).options(
            joinedload(Task.course).joinedload(Course.enrollments) 
        ...
        has_blockers = db.query(exists().where(
            and_(
        ...
        if enrollment and enrollment.target_score is not None:
            ...
            target_gap = max(1.0, enrollment.target_score - (enrollment.current_score or 0.0))

        # Urgency Calculation
        # Ensure task.deadline is a date object
        ...
        # Formula
        impact = task.grade_impact or 1.0
